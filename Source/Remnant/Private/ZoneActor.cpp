...
void AZoneActor::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
    ...
    DOREPLIFETIME(AZoneActor, ItemLevel);
    DOREPLIFETIME(AZoneActor, ParentZoneID);
    ...
    DOREPLIFETIME(AZoneActor, ColorScheme);
    DOREPLIFETIME(AZoneActor, TileSet);
    DOREPLIFETIME(AZoneActor, DayLength);
    DOREPLIFETIME(AZoneActor, DayOffset);
